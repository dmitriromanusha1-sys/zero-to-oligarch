...
# ── Наследие и память (Фаза 21) ───────────────────────────────────────────────
var legacy: float = 0.0            # очки наследия династии (копятся между поколениями)
var legacy_projects: Array = []    # построенные монументы (id)
const CHARITY_TIERS: Array = [
	{"id":"donate", "name":"Пожертвование",  "cost":1_000_000,   "legacy":2.0,  "rep":1.0},
	{"id":"grant",  "name":"Крупный взнос",   "cost":20_000_000,  "legacy":15.0, "rep":2.0},
	{"id":"patron", "name":"Меценатство",     "cost":100_000_000, "legacy":50.0, "rep":4.0},
]
const LEGACY_PROJECTS: Array = [
	{"id":"scholarship", "name":"Стипендиальный фонд",      "cost":50_000_000,    "legacy":40.0,  "prestige":4.0,  "desc":"Поддержка талантливой молодёжи."},
	{"id":"foundation",  "name":"Благотворительный фонд",   "cost":200_000_000,   "legacy":100.0, "prestige":8.0,  "desc":"Фонд вашего имени."},
	{"id":"hospital",    "name":"Больница вашего имени",    "cost":500_000_000,   "legacy":180.0, "prestige":12.0, "desc":"Спасает тысячи жизней."},
	{"id":"university",  "name":"Университет вашего имени",  "cost":2_000_000_000, "legacy":400.0, "prestige":20.0, "desc":"Знания на века."},
]
...
# Игрок — мужчина, партнёр — женщина (традиционный брак). Имена ~90% русские.
const DATING_NAMES: Array = [
	"Анна", "Мария", "Екатерина", "Ольга", "Наталья", "Елена", "Татьяна",
	"Ирина", "Юлия", "Анастасия", "Дарья", "Виктория", "Полина", "Ксения",
	"Алина", "Вера", "София", "Валерия", "Марина", "Кристина",
	"Эмма", "Лейла",
]
...
func happiness_baseline() -> float:
	...
	b += minf(legacy * 0.02, 10.0)                               # гордость за наследие
...
# ── Наследие и память ─────────────────────────────────────────────────────────
func add_legacy(amount: float) -> void:
	legacy = maxf(0.0, legacy + amount)
	emit_signal("life_changed")

func legacy_rank() -> String:
	if legacy >= 1000.0: return "Легендарная династия"
	if legacy >= 500.0:  return "Великая династия"
	if legacy >= 200.0:  return "Знатный род"
	if legacy >= 50.0:   return "Уважаемый род"
	if legacy >= 10.0:   return "Восходящий род"
	return "Безвестный род"

func _charity(id: String) -> Dictionary:
	for c in CHARITY_TIERS:
		if c.id == id: return c
	return {}

func can_donate_charity(id: String) -> bool:
	var c := _charity(id)
	return not c.is_empty() and gm.money >= float(c.cost)

func donate_charity(id: String) -> bool:
	var c := _charity(id)
	if c.is_empty(): return false
	if not gm.spend_money(int(c.cost)): return false
	add_legacy(float(c.legacy))
	social_rep = clampf(social_rep + float(c.rep), 0.0, 100.0)
	add_happiness(2.0)
	emit_signal("life_changed")
	gm.save_game()
	return true

func _project(id: String) -> Dictionary:
	for p in LEGACY_PROJECTS:
		if p.id == id: return p
	return {}

func has_project(id: String) -> bool:
	return id in legacy_projects

func can_build_project(id: String) -> bool:
	var p := _project(id)
	return not p.is_empty() and not has_project(id) and gm.money >= float(p.cost)

func build_project(id: String) -> bool:
	var p := _project(id)
	if p.is_empty() or has_project(id): return false
	if not gm.spend_money(int(p.cost)): return false
	legacy_projects.append(id)
	add_legacy(float(p.legacy))
	add_happiness(5.0)
	var es := get_node_or_null("/root/EventSystem")
	if es:
		es.event_triggered.emit({
			"text": "🏛 Построено: «%s». Ваше имя останется в истории." % p.get("name", ""),
			"money": 0, "health": 0})
	emit_signal("life_changed")
	gm.save_game()
	return true

func legacy_prestige() -> float:
	var t: float = 0.0
	for id in legacy_projects:
		t += float(_project(id).get("prestige", 0.0))
	return t
...
func die() -> void:
	...
	# Наследие за прожитое поколение: база + масштаб состояния
	var nw: float = gm.get_net_worth()
	var scale: float = clampf((log(maxf(1.0, nw)) / log(10.0) - 6.0) * 8.0, 0.0, 80.0)
	add_legacy(10.0 + scale)
...
func social_baseline() -> float:
	...
	b += legacy_prestige()                  # монументы вашего имени
...
func save(cfg: ConfigFile) -> void:
	...
	cfg.set_value("life", "legacy", legacy)
	cfg.set_value("life", "legacy_projects", legacy_projects)
...
func load_data(cfg: ConfigFile) -> void:
	...
	legacy = cfg.get_value("life", "legacy", 0.0)
	legacy_projects = cfg.get_value("life", "legacy_projects", [])
