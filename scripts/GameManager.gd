...
func add_work_income(amount: float, apply_productivity: bool = true) -> void:
	# Счастье/мотивация влияет на продуктивность работы; работа добавляет стресс.
	# Контрактный оклад уже учитывает свой коэффициент эффективности и грейд,
	# поэтому для него продуктивность не накладывается (без двойного учёта).
...
func record_meal(quality: float, weight: float) -> void:
	...
	# Повар извлекает из еды больше пользы — рацион питательнее
	var prof := get_node_or_null("/root/ProfessionManager")
	if prof and prof.has_method("nutrition_quality_bonus"):
		quality += prof.nutrition_quality_bonus()
...
func next_day() -> void:
	...
	if day % 30 == 0:
		...
		if taxable > 0.0:
			var prof_tax := get_node_or_null("/root/ProfessionManager")
			var law_mult: float = prof_tax.wage_tax_mult() if prof_tax else 1.0
			var wage_tax: float = taxable * WAGE_TAX_RATE * tax_mult * law_mult
...
func get_housing_energy_drain_bonus() -> float:
	...
	var prof := get_node_or_null("/root/ProfessionManager")
	var prof_relief: float = prof.work_energy_perk() if prof else 0.0
	return clampf(housing + skill_relief + prof_relief, 0.0, 0.60)
...
func shop_price(base: float) -> int:
	...
	# Профессия: бухгалтер/водитель удешевляют повседневные расходы
	var prof := get_node_or_null("/root/ProfessionManager")
	var prof_exp: float = prof.expense_mult() if prof else 1.0
	return int(round(base * idx * col * shock * pol * prof_exp))
