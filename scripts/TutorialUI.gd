...
func _define_steps() -> void:
	_steps = [
		...
		},
		{
			...
			"text": "🕶 Есть и быстрый, но опасный путь — открой «Криминал» в нижней панели. Чёрный рынок, тёмные дела, рэкет, своя братва и контроль районов приносят «грязные» деньги (их надо отмывать через бизнесы). Но каждое дело поднимает 🚨 розыск: при высоком — облавы, изъятия и тюрьма. Взятки и связи в полиции остужают розыск, а профессия «Юрист» смягчает срок. Риск против куша — решать тебе.",
			"mode": "info",
			"target": "none",
		},
		{
