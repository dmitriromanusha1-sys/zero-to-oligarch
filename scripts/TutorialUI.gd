...
func _define_steps() -> void:
	_steps = [
		...
		},
		{
			...
			"text": "Стоя рядом со зданием, нажми E. На работе выбираешь длину смены (4–12 ч): дольше — больше денег, но сильнее тратятся энергия и здоровье. Чем больше смен отработаешь, тем выше квалификация и надбавка к ставке за выслугу 💼. А хорошая форма и воля (ветка «Жизнь») снижают усталость от смены.",
			"mode": "info",
			"target": "job",
		},
		{
